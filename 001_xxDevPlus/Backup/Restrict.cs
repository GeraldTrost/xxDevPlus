...
using ndString;
 ...
 internal class Restrict //Deprecated, should be replaced with ndBase.store
 {
  ...
  private static bool selfTested  = false;  private static string ass(bool expr) { if (!expr) throw new Exception(ass(true) + " SelfTest Failure"); return "Restrict"; } private static void selfTest() { selfTested = true; } private void init() { if (!selfTested) selfTest(); }
