...
public final class TypeResolver
{
 ...
 public static void enableCache() { cacheEnabled = true; }

 /** Disables the internal caching of TypeVariables. */
 public static void disableCache() { typeVariableCache.clear(); cacheEnabled = false; }
  ...
  */
 public static <T, I extends T> Class<?>[] resolveArguments(Class<I> initialType, Class<T> targetType) { return resolveArguments(resolveGenericType(initialType, targetType), initialType); }
 ...
 public static Class<?>[] resolveArguments(Type genericType, Class<?> targetType)
 {
  ...
  else  if (genericType instanceof TypeVariable) { result = new Class[1]; result[0] = resolveClass(genericType, targetType); }
  return result;
 }
 ...
 {
  Class<?> rawType;
  if (initialType instanceof ParameterizedType) rawType = (Class<?>) ((ParameterizedType) initialType).getRawType(); else rawType = (Class<?>) initialType;
   if (targetType.equals(rawType)) return initialType;
   Type result;
   if (targetType.isInterface()) for (Type superInterface : rawType.getGenericInterfaces()) if (superInterface != null && !superInterface.equals(Object.class)) if ((result = resolveGenericType(superInterface, targetType)) != null) return result;
 ...
 public static Class<?> resolveClass(Type genericType, Class<?> targetType)
 {
  if (genericType instanceof Class) return (Class<?>) genericType;
   ...
   if (genericType instanceof ParameterizedType) return resolveClass(((ParameterizedType) genericType).getRawType(), targetType);
   else
    if (genericType instanceof GenericArrayType) { GenericArrayType arrayType = (GenericArrayType) genericType; Class<?> compoment = resolveClass(arrayType.getGenericComponentType(), targetType); return Array.newInstance(compoment, 0).getClass(); }
    else
     if (genericType instanceof TypeVariable)  { TypeVariable<?> variable = (TypeVariable<?>) genericType; genericType = getTypeVariableMap(targetType).get(variable); genericType = genericType == null ? resolveBound(variable) : resolveClass(genericType, targetType);}
 ...
 static void buildTypeVariableMap(final Type[] types, final Map<TypeVariable<?>, Type> map)
 {
  for (Type type : types)
  {
   if (type instanceof ParameterizedType) { ParameterizedType parameterizedType = (ParameterizedType) type; buildTypeVariableMap(parameterizedType, map); Type rawType = parameterizedType.getRawType(); if (rawType instanceof Class) buildTypeVariableMap(((Class<?>) rawType).getGenericInterfaces(), map); }
   else if (type instanceof Class) buildTypeVariableMap(((Class<?>) type).getGenericInterfaces(), map);
  }
 }
 ...
 private static void buildTypeVariableMap(ParameterizedType type, Map<TypeVariable<?>, Type> typeVariableMap)
 {
  if (type.getRawType() instanceof Class)
  {
   ...
   for (int i = 0; i < typeArguments.length; i++)
   {
    ...
    if (typeArgument instanceof Class) typeVariableMap.put(variable, typeArgument);
    else
     if (typeArgument instanceof GenericArrayType) typeVariableMap.put(variable, typeArgument);
     else
      if (typeArgument instanceof ParameterizedType) typeVariableMap.put(variable, typeArgument);
      else if (typeArgument instanceof TypeVariable) { TypeVariable<?> typeVariableArgument = (TypeVariable<?>) typeArgument; Type resolvedType = typeVariableMap.get(typeVariableArgument); if (resolvedType == null) resolvedType = resolveBound(typeVariableArgument); typeVariableMap.put(variable, resolvedType); }
   }
  }
 }
