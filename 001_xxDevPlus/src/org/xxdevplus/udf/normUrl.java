...
public class normUrl
{
 ...
 private static boolean selfTested  = false;  private static String ass(boolean expr) throws Exception { if (!expr) throw new Exception(ass(true) + " SelfTest Failure"); return "ndUrl"; } private static void selfTest() { selfTested = true; } private void init() { if (!selfTested) selfTest(); }
 ...
 public Chain Domain;
 public Chain User;
 public Chain Password;
 public Chain Server;
 public Chain Extension;
 public Chain Port;
 public Chain Path;
 public Chain File;
 public Assoc Query;

 public String text () throws Exception { String ret = Protocol.text() + "://"; if (User.len() + Password.len() > 0) ret += User.text() + ":" + Password.text() + "@"; if (Port.len() > 0) ret += Server.text() + "." + Domain.text() + "." + Extension.text() + ":" + Port.text() + "/" + Path.text() + "/" + File.text(); else ret+=Server.text() + "." + Domain.text() + "." + Extension.text() + "/" + Path.text() + "/" + File.text(); if (Query.Len() > 0) ret += "?" + Query.text(); return ret; }


 public normUrl(String urlString) throws Exception
 {
  urlString = urlString.trim();
  if (urlString.indexOf("://") < 0) urlString = "http://" + urlString;
  _url = new Chain(urlString);
